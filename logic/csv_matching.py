...
def csv_matching(
    ...
    try:
        # CSVファイルを開く
        user_file = open(user_list_csv_path, "r", encoding="utf-8")
        address_file = open(address_list_csv_path, "r", encoding="utf-8")
    except ValueError:
        user_file = open(user_list_csv_path, "r", encoding="cp932")
        address_file = open(address_list_csv_path, "r", encoding="cp932")
    except UnicodeDecodeError:
        return "CSVファイルの文字コードが不正です"
    except FileNotFoundError:
        return "CSVファイルが見つかりません"
    finally:
        if user_file is None or address_file is None:
    ...
    try:
        ...
        for key, mapped_col in matching_value_map.items():
            if mapped_col == "":
                ...
                if key in address_list_csv_header:
                    address_list_csv_index.append(address_list_csv_header.index(key))
                else:
        ...
        address_list_csv_target_values = [
            [row[index] for index in address_list_csv_index] for row in address_list_csv
        ]
        ...
        no_match_keys = date_logic.dete_format_rows(no_match, address_csv_date_format)
        one_match_keys = date_logic.dete_format_rows(one_match, address_csv_date_format)
